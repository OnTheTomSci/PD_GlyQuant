...
########################################################
# First, get unique protein accessions
unique_proteins <- unique(glycoPSMs$protein_accessions)

# Function to create analysis for one protein if data is sufficient
analyze_protein <- function(protein_acc) {
  # Subset data for this protein
  protein_data <- glycoPSMs %>%
    filter(protein_accessions == protein_acc)
  
  # Check if we have data for all samples in both groups
  sample_coverage <- protein_data %>%
    group_by(disease_status) %>%
    summarise(
  ...
  # Only proceed if we have data for both groups
  if (nrow(sample_coverage) == 2 && all(sample_coverage$n_samples > 0)) {
    # Calculate fucosylation percentages
    fuc_by_sample <- protein_data %>%
      group_by(sample, disease_status) %>%
      summarise(
        fuc_intensity = sum(intensity[contains_Fuc == TRUE], na.rm = TRUE),
        total_intensity = sum(intensity, na.rm = TRUE),
        relative_percentage = (fuc_intensity / total_intensity) * 100,
        .groups = 'drop'
      )
    
    # Only proceed if we have valid percentages
    if (nrow(fuc_by_sample) > 0 && !all(is.na(fuc_by_sample$relative_percentage))) {
      # Perform t-test
      t_test <- try(t.test(relative_percentage ~ disease_status, data = fuc_by_sample), silent = TRUE)
      
      if (!inherits(t_test, "try-error")) {
        p_val <- sprintf("p = %.3g", t_test$p.value)
        
        # Create boxplot
        plot <- ggplot(fuc_by_sample, 
                      aes(x = disease_status, 
                          y = relative_percentage, 
                          fill = disease_status)) +
          geom_boxplot(alpha = 0.7, outlier.shape = NA) +
          ...
          labs(
            title = paste0("Fucosylation % in ", protein_acc),
            x = "Disease Status",
            y = "Relative Abundance (%)"
          ) +
          annotate("text", 
                  x = 1.5, 
                  y = max(fuc_by_sample$relative_percentage, na.rm = TRUE) * 1.05,
                  label = p_val)
        
        # Save plot
        ggsave(
          sprintf("figures/fucosylation_%s_boxplot.pdf", protein_acc),
          plot,
          width = 6,
          height = 5,
          dpi = 300
        ...
        ggsave(
          sprintf("figures/fucosylation_%s_boxplot.png", protein_acc),
          plot,
          width = 6,
          height = 5,
          dpi = 300
...
for (protein in unique_proteins) {
  ...
  if (!is.null(result)) {
    ...
    valid_plots[[protein]] <- result$plot
    
    # Print summary statistics
    cat(sprintf("\nResults for protein %s:\n", protein))
...
# Combine all valid plots if there are any
if (length(valid_plots) > 0) {
  ...
  # Combine plots
  combined_plot <- wrap_plots(valid_plots, ncol = n_cols)
  
  # Save combined plot
  ggsave("figures/all_proteins_fucosylation_boxplots.pdf", 
         combined_plot, 
         width = 6 * n_cols, 
         height = 5 * n_rows, 
         dpi = 300)
  
  ggsave("figures/all_proteins_fucosylation_boxplots.png", 
         combined_plot, 
         width = 6 * n_cols, 
         height = 5 * n_rows, 
         dpi = 300)
...
if (length(protein_analyses) > 0) {
  all_stats <- bind_rows(
    lapply(protein_analyses, function(x) {
      x$summary_stats %>%
        mutate(protein = x$protein)
    })
  )
  
  write.csv(all_stats, 
            "output_data/protein_fucosylation_summary_stats.csv", 
            row.names = FALSE)
}

# Filter for significant proteins and create combined plot
significant_plots <- list()

for (protein in names(protein_analyses)) {
  # Check if t-test result exists and p-value is valid
  if (!is.null(protein_analyses[[protein]]$t_test_result) && 
      !is.na(protein_analyses[[protein]]$t_test_result$p.value) && 
      protein_analyses[[protein]]$t_test_result$p.value < 0.05) {
    
    # Get p-value
    p_val <- protein_analyses[[protein]]$t_test_result$p.value
    
    # Create significance symbol
    sig_symbol <- if(p_val < 0.001) "***"
                  else if(p_val < 0.01) "**"
                  else "*"
    
    # Add plot to significant plots list
    significant_plots[[protein]] <- protein_analyses[[protein]]$plot +
      annotate("text",
               x = 1.5,
               y = max(protein_analyses[[protein]]$summary_stats$mean_percentage) * 1.1,
               label = sig_symbol)
  }
}

# Combine significant plots if there are any
if (length(significant_plots) > 0) {
  library(patchwork)
  
  # Calculate number of rows and columns for grid
  n_plots <- length(significant_plots)
  n_cols <- min(3, n_plots)
  n_rows <- ceiling(n_plots / n_cols)
  
  # Combine plots
  significant_combined_plot <- wrap_plots(significant_plots, ncol = n_cols)
  
  # Save combined plot
  ggsave("figures/significant_proteins_fucosylation_boxplots.pdf", 
         significant_combined_plot, 
         width = 6 * n_cols, 
         height = 5 * n_rows, 
         dpi = 300)
  
  ggsave("figures/significant_proteins_fucosylation_boxplots.png", 
         significant_combined_plot, 
         width = 6 * n_cols, 
         height = 5 * n_rows, 
         dpi = 300)
  ...
  significant_stats <- bind_rows(
    lapply(names(significant_plots), function(protein) {
      data.frame(
        protein = protein,
        p_value = protein_analyses[[protein]]$t_test_result$p.value,
        mean_healthy = mean(protein_analyses[[protein]]$summary_stats$mean_percentage[protein_analyses[[protein]]$summary_stats$disease_status == "Healthy"]),
        mean_MECFS = mean(protein_analyses[[protein]]$summary_stats$mean_percentage[protein_analyses[[protein]]$summary_stats$disease_status == "MECFS"]),
        fold_change = mean(protein_analyses[[protein]]$summary_stats$mean_percentage[protein_analyses[[protein]]$summary_stats$disease_status == "MECFS"]) /
                     mean(protein_analyses[[protein]]$summary_stats$mean_percentage[protein_analyses[[protein]]$summary_stats$disease_status == "Healthy"])
      )
    })
  ) %>%
    arrange(p_value)
  ...
  # Save significant proteins statistics
  write.csv(significant_stats,
            "output_data/significant_protein_fucosylation_stats.csv",
            row.names = FALSE)
  
  # Print number of significant proteins
  cat(sprintf("\nNumber of proteins with significant differences: %d\n", n_plots))
} else {
  cat("\nNo proteins showed significant differences in fucosylation.\n")
}

# Print diagnostic information
cat("\nDiagnostic Information:\n")
cat("Total number of proteins analyzed:", length(protein_analyses), "\n")
cat("Number of proteins with valid t-tests:", 
    sum(!sapply(protein_analyses, function(x) is.null(x$t_test_result))), "\n")
